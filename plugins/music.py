...
class Music163():
    ...
    def get_music_list(self, keywords):
        ...
        for offset in range(1):
            result = Music163().search(keywords, str(offset))
...
def get_music_id(keywords: str, site: str="163") -> str:
    if site == "163":
        music_list = Music163().get_music_list(keywords)
        if len(music_list):
            return music_list[0]['id']
    elif site == "qq":
        w = parse.urlencode({'w': keywords})
        url = "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?"\
                "ct=24&qqmusic_ver=1298&new_json=1&remoteplace=txt.yqq.top&"\
                "searchid=1&t=0&aggr=1&cr=1&catZhida=1&lossless=0&flag_qc=0"\
                f"&p=1&n=10&{w}&_=1621238658364&cv=4747474&ct=24&format=json&"\
                "inCharset=utf-8&outCharset=utf-8&notice=0&platform=yqq.json&"\
                "needNewCode=0&uin=0&g_tk_new_20200303=5381&g_tk=5381&hostUin=0&loginUin=0"
        ...
        if data["code"] != 0:
            return None
        try:
            song_id = str(data['data']['song']['list'][0]['id'])
